...
is_int() [[ "$1" =~ ^([0-9]|-[0-9]*[1-9])[0-9]*$ ]]

# is the argument a non-negative integer?
is_nnint() [[ "$1" =~ ^[0-9][0-9]*$ ]]
...
argcheck_is_int() { _argcheck_is "is not an integer" "$@" is_int; }
argcheck_is_nnint() { _argcheck_is "is not a non-negative integer" "$@" is_nnint; }
...
argcheck_all_int() { _argcheck_all "is not an integer" "$@" is_int; }
argcheck_all_nnint() { _argcheck_all "is not a non-negative integer" "$@" is_nnint; }
